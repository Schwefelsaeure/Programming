...
{
	char default_filename[] = "log_mqx.bin";
	const char *filename;
...
void handle_data_entry()
{
	...
	printf("%u, ", DATA_ENTRY);
	printf("%u, ", data_entry.seconds);
	printf("%u, ", data_entry.milliseconds);
	printf("%u, ", data_entry.key);
	printf("0x%X", data_entry.value);
	printf("\n");
}
...
void handle_message_entry()
{
	...
	printf("%u, ", message_entry.seconds);
	printf("%u, ", message_entry.milliseconds);
...
void print_usage()
{
	printf("Usage: log_parser_mqx [file]\n");
	printf("\n");
	printf("Note: log_parser_mqx uses the default file \"log_mqx.bin\".\n");
}
